...
現在、Chaosdは`cache-expiration`を実行したキーの復旧をサポートしていないため、復旧が必要な場合は事前にバックアップしてください。
...
2. Chaosdサービスの`/api/attack/redis`パスに`POST` HTTPリクエストを送信します。
   ...
   上記のコマンドでは、`fault-configuration`を障害タイプに応じて設定する必要があります。対応するパラメータについては、以下のセクションの各障害タイプのパラメータと例を参照してください。
...
#### サービスモードでのキャッシュペネトレーションシミュレーション例
